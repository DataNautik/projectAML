...
class DDPM(nn.Module):
    ...
    def negative_elbo(self, x: torch.Tensor) -> torch.Tensor:
        """
        Per-sample DDPM training objective (B,).
        """
        B = x.shape[0]
        device = x.device
        ...
        a_bar = self.alpha_cumprod[t]  # (B,)
        a_bar = a_bar.view(B, *([1] * (x.dim() - 1)))  # (B,1,...,1)

        x_t = torch.sqrt(a_bar) * x + torch.sqrt(1.0 - a_bar) * eps

        # network expects time as (B,1) in [0,1]
        t_norm = (t.float() / (self.T - 1)).view(B, 1)

        eps_hat = self.network(x_t, t_norm)

        mse = (eps - eps_hat).reshape(B, -1).pow(2).sum(dim=1)  # (B,)
        return self.T * mse

    def loss(self, x):
        """
        Scalar loss used by ddpm.train(...).
        """
    ...
    def sample(self, shape):
        """
        Reverse diffusion sampling, sigma_t^2 = beta_t.
        """
        ...
        for t in range(self.T - 1, -1, -1):
            ...
            t_norm = (t_batch.float() / (self.T - 1)).view(B, 1)
            ...
            mu = (1.0 / torch.sqrt(alpha_t)) * (
                x_t - (beta_t / torch.sqrt(1.0 - a_bar_t)) * eps_hat
            )
